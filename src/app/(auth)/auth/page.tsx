...
import { ButtonLoader, errorToast } from "@/utils/customToast";
...
const AuthForms = () => {
  ...
  const { login, isAuthenticated, register, loading } = useAuthStore();
  ...
  const handleLogin = async (e: AuthEvent) => {
    ...
    try {
      ...
      console.log(error);
      errorToast(error?.message ? error?.message : "Login failed");
    }
  };
            ...
            <CardContent>
              <form onSubmit={handleLogin}>
                ...
                <CardFooter className="flex justify-end mt-4 px-0">
                  <Button
                    type="submit"
                    className={`bg-violet-500 hover:bg-violet-600 ${loading && "cursor-not-allowed disabled"}`}
                  >
